...
export default function ProfileSettings() {
    ...
    return (
        <div className={`bg-base-300 p-10 flex-1`}>
            <p className={`text-base-content mb-1.5`}>Language</p>
            <div className={`relative w-1/2 mb-2.5`}>
                <img onClick={() => setLanguageSelectOpen(!languageSelectOpen)} src={languageData.find(val => val.name === selectedLanguage).image} alt='United States' className={`w-5 aspect-square cursor-pointer rounded-full absolute top-3 start-2.5`} />
                ...
                <p onClick={() => setLanguageSelectOpen(!languageSelectOpen)} className={`pl-10 px-4 py-2 rounded-md border border-neutral w-full cursor-pointer hover:border-base-200 hover:shadow-lg transition-all ease-in-out duration-200`}>{selectedLanguage}</p>
                ...
                <AnimatePresence>
                    {languageSelectOpen && (
                        <motion.div key={languageSelectOpen} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ type: "spring", duration: 0.6 }} className={`w-full z-[100] absolute mt-3 shadow-xl rounded-md border border-neutral bg-base-300 bg-opacity-100`}>
                            {languageData.filter(language => language.name !== selectedLanguage).map((option, index) => {
                                return (
                                    <div onClick={() => {setSelectedLanguage(option.name); setLanguageSelectOpen(false);}} key={index} className={`flex items-center gap-2 py-2 px-2.5 hover:bg-neutral hover:bg-opacity-40 transition-all duration-200 ease-in-out cursor-pointer`}>
            ...
            <p className={`text-base-content mb-1.5`}>Currency</p>
            <div className={`relative w-1/2 mb-10 z-50`}>
                ...
                <AnimatePresence>
                    {currencySelectOpen && (
                        <motion.div key={currencySelectOpen} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ type: "spring", duration: 0.6 }} className={`w-full absolute mt-3 shadow-xl rounded-md border border-neutral bg-base-300 bg-opacity-100`}>
            ...
            <p className={`text-xl font-bold text-white mb-3 flex items-center gap-3`}>Notification Settings<input type='checkbox' className={`toggle toggle-success toggle-sm`} /></p>
            <p className={`font-medium text-base-content text-lg mb-2.5 flex items-center gap-2 w-full`}>Trades<div className={`h-px rounded-full bg-base-content w-full mt-0.5`}></div></p>
            <div className={`flex items-center justify-between w-full`}>
                <div>
                    <p className={`font-medium text-white`}>Trade Executed</p>
                    <p className={`font-normal text-base-content mb-2.5`}>Recieve an alert when a trade has been completed.</p>
                </div>
                <input type='checkbox' className={`toggle toggle-success toggle-sm`}/>
            </div>
            <p className={`font-medium text-white`}>Trade Rejected</p>
            <p className={`font-normal text-base-content mb-10`}>Recieve an alert when a trade order fails.</p>
